/**
 * @file canopen_test_fixture.hpp
 * @brief Test fixture with automatic mock motor responder for CANopen integration tests
...
#include "test_socket_helper.hpp"
#include <thread>
#include <atomic>
#include <linux/can.h>
    ...
    class MockMotorResponder {
        public:
            explicit MockMotorResponder(uint8_t node_id, const std::string& interface = "vcan0")
                : node_id_(node_id)
                , running_(false)
                , socket_(waveshare::make_real_socket(interface)) {
...
/**
 * @brief RAII fixture for CANopen integration tests
 *
 * Automatically starts/stops mock motor responder.
 * Usage with Catch2:
 *
 * TEST_CASE_METHOD(CANopenTestFixture, "My test", "[integration]") {
 *     // mock_motor is already running
 *     auto socket = waveshare::make_real_socket("vcan0");
 *     SDOClient client(socket, dict, 1);
 *     // ... test code ...
 * }
 */
    class CANopenTestFixture {
        protected:
            CANopenTestFixture(uint8_t node_id = 1)
                : mock_motor(node_id) {
            ...
            static bool is_vcan_available() {
                try {
                    auto test_socket = waveshare::make_real_socket("vcan0");
