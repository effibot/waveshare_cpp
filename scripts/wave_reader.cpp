...
void display_help(std::string argv) {
    ...
    std::cout << "\t-f Fixed/variable frame to read (default: variable)\n";
    std::cout << "\t-h Display this help message\n";
}

int main(int argc, char* argv[]) {
    // Default parameters
    std::string device = "/dev/ttyUSB0"; // Default device
    ...
    // Create USBAdapter instance
    USBAdapter* adapter = new USBAdapter(device, baudrate);
    std::cout << "Adapter initialized: " << adapter->to_string() << "\n";
    // Send the config frame to set the adapter
    ...
    while (!USBAdapter::should_stop()) {
        // Read either fixed or variable frames based on user choice
